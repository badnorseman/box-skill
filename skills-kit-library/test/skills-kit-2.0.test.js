...
describe("SkillsErrorEnum", () => {
  test("skills errors", () => {
    expect(SkillsErrorEnum.FILE_PROCESSING_ERROR).toEqual(
      "skills_file_processing_error"
    );
    expect(SkillsErrorEnum.INVALID_FILE_SIZE).toEqual(
      "skills_invalid_file_size_error"
    );
    expect(SkillsErrorEnum.INVALID_FILE_FORMAT).toEqual(
    ...
    expect(SkillsErrorEnum.INVOCATIONS_ERROR).toEqual(
      "skills_invocations_error"
    );
    expect(SkillsErrorEnum.EXTERNAL_AUTH_ERROR).toEqual(
      "skills_external_auth_error"
    );
...
describe("FilesReader", () => {
  ...
  test("getContentStream() should return content stream", done => {
    reader.fileReadClient = {
      files: {
        getReadStream: jest
          .fn()
          .mockReturnValue(Promise.resolve(mockFileStream))
      }
    };

    reader.getContentStream().then(stream => {
      expect(stream).toEqual(mockFileStream);
      expect(reader.fileReadClient.files.getReadStream).toHaveBeenCalledWith(
        fileId,
        null,
        expect.anything()
      );
  ...
  test("getContentBase64() should throw exception for invalid image stream", () => {
    ...
    reader.getContentStream = jest
      .fn()
      .mockReturnValue(Promise.resolve(invalidFileStream));
  ...
  test("getBasicFormatContentStream() should return representation content", () => {
    ...
    return reader.getBasicFormatContentStream().then(() => {
      expect(
        reader.fileReadClient.files.getRepresentationContent
      ).toHaveBeenCalledWith(fileId, "[mp3]");
    });
  });
  ...
  test("getBasicFormatContentBase64() should return base64 encoded content in basic format", () => {
    reader.getBasicFormatContentStream = jest
      .fn()
      .mockReturnValue(Promise.resolve(mockFileStream));
  ...
  test("getBasicFormatFileURL() should throw exception if requested representation not found", () => {
    ...
    reader.fileReadClient = {
      files: {
        getRepresentationInfo: jest
          .fn()
          .mockReturnValue(Promise.resolve(missingReps))
      }
    };
  ...
  test("getBasicFormatFileURL() should throw exception for unknown representation status", () => {
    ...
    return reader.getBasicFormatFileURL().catch(error => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        "Representation had error status"
      );
    });
  });
});
...
describe("SkillsWriter", () => {
  ...
  test("createMetadataCard() should return metadata card", () => {
    ...
    const metadataCard = writer.createMetadataCard(
      "keyword",
      "Topics",
      status,
      entries,
  ...
  test("createTranscriptsCard() should return transcript card", () => {
    ...
    const transcriptCard = writer.createTranscriptsCard(
      transcriptsDataList,
      duration
    );
  ...
  test("saveProcessingCard() should save processing card", () => {
    ...
    expect(writer.createMetadataCard).toHaveBeenCalledWith(
      "status",
      "Status",
      status
    );
    expect(writer.saveDataCards).toHaveBeenCalledWith(
      [statusCard],
      callback,
      "processing"
    );
  ...
  test("saveErrorCard() should save error card with one of the default codes from error enum", () => {
    ...
    writer.saveErrorCard(
      SkillsErrorEnum.FILE_PROCESSING_ERROR,
      undefined,
      callback
    );
  ...
  test("saveDataCards() should save data cards", () => {
    ...
    writer.fileWriteClient.wrapWithDefaultHandler = jest
      .fn()
      .mockReturnValue(putFunc);

    writer.saveDataCards(dataCardsJson, callback);

    expect(writer.fileWriteClient.wrapWithDefaultHandler).toHaveBeenCalledWith(
      writer.fileWriteClient.put
    );
